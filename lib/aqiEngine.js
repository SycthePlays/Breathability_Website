...
import { fetchLiveAqiBatch, forecastAt } from './openMeteo';
import { REFERENCE_AREAS, isInServiceArea } from './jakartaAreas';
...
const LIVE_ANCHORS = [
  ...
  // Banten
  { name: 'Kota Tangerang', lat: -6.1780, lon: 106.6320 },
  { name: 'Tangerang Selatan', lat: -6.3018, lon: 106.7100 },
  { name: 'Kabupaten Tangerang', lat: -6.2200, lon: 106.5100 },
  { name: 'Serang', lat: -6.1200, lon: 106.1500 },
  { name: 'Cilegon', lat: -6.0170, lon: 106.0540 },
  { name: 'Pandeglang-Lebak', lat: -6.3400, lon: 106.1800 },
];
...
function interpolateSlowSignals(lat, lon, areas) {
  if (!isInServiceArea(lat, lon) || !areas.length) {
...
//
// opts.atTime (Date/ISO, optional): score with the CAMS hourly FORECAST
// for that time instead of the current reading. The forecast rides in the
// same cached anchor entry, so predictions cost no extra quota.
export async function getCompositeAqiBatch(points, opts = {}) {
  ...
  const anchored = points.map((p) =>
    isInServiceArea(p.lat, p.lon) ? nearestAnchor(p.lat, p.lon) : { lat: p.lat, lon: p.lon }
  );
  ...
  const atTime = opts.atTime ? new Date(opts.atTime) : null;

  return points.map((p, i) => {
    const live = liveByKey.get(`${anchored[i].lat},${anchored[i].lon}`);
    // For a future departure, swap the live reading for the forecast at
    // that hour before weighting.
    const usAqi = atTime ? forecastAt(live, atTime) : live?.usAqi ?? null;
    const slow = interpolateSlowSignals(p.lat, p.lon, areas);
    const result = {
      lat: p.lat,
      lon: p.lon,
      compositeScore: compositeScore({ usAqi }, slow),
      liveAqi: usAqi,
      forecastFor: atTime ? atTime.toISOString() : null,
      pm25: live?.pm25 ?? null,
      pm10: live?.pm10 ?? null,
      signals: {
    ...
    if (!atTime) persistToCache(result); // cache table holds current readings only
    return result;
  });
}
...
// Hourly composite forecast for one point: the next `hours` hours scored
// with the same four-signal formula (slow signals interpolated once, CAMS
// forecast supplying the live term per hour), plus the cleanest upcoming
// hour as a "best window" suggestion. Costs at most one anchor fetch —
// usually zero, since the anchor is already cached from the route search
// that triggered this.
export async function getHourlyForecast(lat, lon, hours = 24) {
  const areas = await getReferenceAreas();
  const slow = interpolateSlowSignals(lat, lon, areas);

  const anchor = isInServiceArea(lat, lon) ? nearestAnchor(lat, lon) : { lat, lon };
  const [entry] = await fetchLiveAqiBatch([anchor]);
  ...
  for (let i = 0; i < hours; i++) {
    ...
    const usAqi = forecastAt(entry, t);
    if (usAqi === null) continue; // past the forecast horizon
    out.push({
      time: t.toISOString(),
      liveAqi: Math.round(usAqi),
      compositeScore: compositeScore({ usAqi }, slow),
    });
  }

  const best = out.reduce((b, h) => (b === null || h.compositeScore < b.compositeScore ? h : b), null);
