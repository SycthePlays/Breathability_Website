...
// `departAt` shifts the live term to the CAMS forecast for that hour.
async function averageExposures(candidates, departAt) {
  ...
  let results;
  try {
    results = await getCompositeAqiBatch(flat, departAt ? { atTime: departAt } : {});
  } catch {
...
export async function getRankedRoutes(start, destination, { departAt = null } = {}) {
  const candidates = await gatherCandidates(start, destination);
  if (!candidates.length) return [];

  const exposures = await averageExposures(candidates, departAt);
